...
class WorkflowState:
    ...
    def save(self, state_file: Path) -> None:
        ...
        state_dict["selected_files"] = (
            [str(p) for p in self.selected_files] if self.selected_files else None
        )
    ...
    def load(cls, state_file: Path) -> "WorkflowState":
        ...
        state_dict["selected_files"] = (
            [Path(p) for p in state_dict["selected_files"]]
            if state_dict["selected_files"]
            else None
        )
        state_dict["processed_file"] = (
            Path(state_dict["processed_file"]) if state_dict["processed_file"] else None
        )
        state_dict["cleaned_file"] = (
            Path(state_dict["cleaned_file"]) if state_dict["cleaned_file"] else None
        )
...
class ThreeStepWorkflow:
    ...
    def step1_select_and_move(
        ...
        filtered_candidates = self._apply_patterns(
            all_candidates, state.include_patterns, state.exclude_patterns
        )
        ...
        if selection_mode == "manifest":
            ...
            for i, src_file in enumerate(final_candidates):
                ...
                try:
                    ...
                    logging.warning(
                        "Failed to symlink %s -> %s (%s); recording manifest path instead",
                        src_file,
                        dst_file,
                        e,
                    )
        ...
        if selection_mode == "manifest":
            logging.info(
                f"Step 1 complete: Selected {len(selected_list)} files (manifest-only, no data copied)"
            )
        else:
            logging.info(
                f"Step 1 complete: Materialized {len(selected_list)} files to {self.selected_dir} via {selection_mode}"
            )
        return state
    ...
    def step3_clean(
        ...
        step_count = 0
        total_steps = sum(
            1 for key in ["snaplen", "convert_to_pcap", "gzip"] if key in options and options[key]
        )
    ...
    def _apply_patterns(
        self, files: List[Path], include_patterns: List[str], exclude_patterns: List[str]
    ) -> List[Path]:
